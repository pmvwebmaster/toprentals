...
Class SliceWP_WP_List_Table_Payments extends SliceWP_WP_List_Table {
	...
	public function column_actions( $item ) {
		 ...
		 */
		$row_actions = array(
			'mark_as_paid' => ( $item['status'] != 'paid' ? '<a onclick="return confirm( \'' . __( "By marking this payment as paid, its status will be changed to paid and all commissions attached to it will also be marked as paid. Are you sure you want to mark this payment as paid?", "slicewp" ) . ' \' )" href="' . esc_url( wp_nonce_url( add_query_arg( array( 'slicewp_action' => 'mark_payment_as_paid', 'payment_id' => $item['id'] ) ), 'slicewp_mark_payment_as_paid', 'slicewp_token' ) ) . '">' . __( 'Mark as Paid', 'slicewp' ) . '</a>' : '' ),
			'delete' 	   => '<a class="slicewp-trash" onclick="return confirm( \'' . __( "Are you sure you want to delete this payment?", "slicewp" ) . ' \' )" href="' . esc_url( wp_nonce_url( add_query_arg( array( 'slicewp_action' => 'delete_payment', 'payment_id' => $item['id'] ) ), 'slicewp_delete_payment', 'slicewp_token' ) ) . '">' . __( 'Delete', 'slicewp' ) . '</a>'
		);
		...
		$output  = '<div class="row-actions">';
			...
			$output .= '<a href="' . esc_url( add_query_arg( array( 'page' => 'slicewp-payouts', 'subpage' => 'review-payment', 'payment_id' => $item['id'] ) , admin_url( 'admin.php' ) ) ) . '" class="slicewp-button-secondary">' . __( 'View', 'slicewp' ) . '</a>';
